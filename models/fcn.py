...
class FCNs(nn.Module):
    ...
    def forward(self, x):
        output = self.pretrained_net(x)
        #print("vgg output",[x.shape for key, x in output.items()])
        x5 = output['x5']  # size=(N, 512, x.H/32, x.W/32)
        x4 = output['x4']  # size=(N, 512, x.H/16, x.W/16)
        x3 = output['x3']  # size=(N, 256, x.H/8,  x.W/8)
...
class FCNmy(nn.Module):
    ...
    def forward(self, x):
        output = self.pretrained_net(x)
        x3 = output['x3']  # size=(N, 256, x.H/8,  x.W/8)
        x2 = output['x2']  # size=(N, 256, x.H/8,  x.W/8)
        x1 = output['x1']  # size=(N, 256, x.H/8,  x.W/8)
...
class VGGNet(VGG):
    def __init__(self, pretrained=False, model='vgg9', xyconv=False, requires_grad=True, remove_fc=True, show_params=False):
        super().__init__(make_layers(cfg[model], xyconv=xyconv))
...
def make_layers(cfg, xyconv=False, batch_norm=False):
    layers = []
    in_channels = 3
    for v in cfg:
        if v == 'M':
            ...
            if xyconv:
                conv2d = Conv2dXY(in_channels, v, kernel_size=3, padding=1)
            else:
...
if __name__ == "__main__":
    batch_size, n_class, h, w = 1, 3, 64, 64

    # test output size
    vgg_model = VGGNet(requires_grad=True).cuda()
    input = torch.autograd.Variable(torch.randn(batch_size, 3, 64, 64)).cuda()
    output = vgg_model(input)
    print("vgg output",[(key,x.shape) for key, x in output.items()])
    print("vgg done")
    ...
    fcn_model = FCNmy(pretrained_net=vgg_model, n_class=n_class).cuda()
    ...
    input = torch.autograd.Variable(torch.randn(batch_size, 3, h, w)).cuda()
    y = torch.autograd.Variable(torch.randn(batch_size, n_class, h, w), requires_grad=False).cuda()
    for iter in range(5):
        optimizer.zero_grad()
        output = fcn_model(input)
